...
/* --gela = CONTROLLO NEGATIVO (solo in questa copia di prova). Spegne la
...
(async () => {
  ...
  const GIOCO = process.argv.find(a => a.endsWith('.html')) || 'CALCETTO-il-gioco.html';
  ...
  /* il cartello spento: stessa cura di folla.js, se no il controllo
     negativo fallirebbe per il motivo sbagliato e mi direbbe di si' */
  await pag.evaluate(() => { const G = window.__test.G; G.banner = ''; G.bannerT = 0; });

  if (GELA) await pag.evaluate(() => {
