/* =====================================================================
   ...
   uso:
     node strumenti/_z-verbo-prova.js --gioco fuori/dopo.html
...
/* --- innesto di _z-verbo-prova.js: la radice resta il repo, ma il file
   di gioco puo' arrivare da fuori (--gioco, o GIOCO_PROVA). Nient'altro
   e' toccato: la sonda, il ciclo e la statistica sono quelli di
   _z-verbo.js, cosi' i numeri restano confrontabili. --- */
const __PROVA = (() => {
  const i = process.argv.indexOf('--gioco');
  const v = (i > 0 && process.argv[i + 1]) ? process.argv[i + 1] : process.env.GIOCO_PROVA;
...
function servi() {
  return new Promise(ok => {
    const s = http.createServer((req, res) => {
      ...
      if (__PROVA && /CALCETTO-il-gioco\.html$/i.test(f)) f = __PROVA;
