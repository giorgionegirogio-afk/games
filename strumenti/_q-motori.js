...
(async () => {
  ...
  try {
    ...
    /* ---- T: i trascendenti nudi ---- */
    console.log('T) I TRASCENDENTI NUDI — 200 valori, funzione per funzione, BIT PER BIT');
    console.log('   (fra parentesi quante volte il gioco chiama quella funzione)');
    const fn = {};
    ...
    for (const k of Object.keys(fn[base])) {
      ...
      di(peggio === 0, 'Math.' + k.padEnd(13) + ' da\' gli stessi bit ovunque', righe.join(' | '));
    }
    console.log('');
