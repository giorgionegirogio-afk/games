...
(async () => {
  ...
  const guarda = async (id) => B.pag.evaluate(async id => {
    const t = window.__test;
    ...
    let r = null;
    for (let g = 0; g < 12 && (!r || r.scene !== 'end'); g++) r = t.simulate(20);
  ...
  if (senza) {
    ...
    const primaRep = await B.pag.evaluate(() => ({ monete: window.__test.save.coins, rosa: (window.__test.save.rosa||[]).map(g=>[g.vel,g.tiro,g.tecnica,g.tackle].join('/')).join(' '), trofei: JSON.stringify(window.__test.save.trofei||window.__test.save.trophies||null) }));
    console.log('    >>> PRIMA DEL REPLAY: monete=' + primaRep.monete + ' rosa=' + primaRep.rosa.slice(0,60));
    const rep = await guarda(senza.id);
    ...
    const dopoRep = await B.pag.evaluate(() => ({
      ...
      rosaDopo: (window.__test.save.rosa||[]).map(g=>[g.vel,g.tiro,g.tecnica,g.tackle].join('/')).join(' '),
    }));
    ...
    console.log('    >>> DOPO IL REPLAY:  monete=' + dopoRep.monete + ' rosa=' + dopoRep.rosaDopo.slice(0,60));
    di(primaRep.monete === dopoRep.monete, 'UN REPLAY NON PAGA: le monete non si muovono', primaRep.monete + ' -> ' + dopoRep.monete);
    di(primaRep.rosa === dopoRep.rosaDopo, 'UN REPLAY NON FA CRESCERE LA ROSA');
  } else {
